...
{
    public partial class FRM_X_RAY : Form
    {
        byte[] imge1;
        byte[] imge2;
        byte[] imge3;
        ...
        private void FRM_IMGE_PRSN_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                LBL_PERS_NAME.Text = CLS.PERSON_NAME_IMGE;
                LBL_PERS_CODE.Text = CLS.CODE_PER_IMG.ToString(); ;
                LBL_BOOK_CODE.Text = CLS.CODE_BOOK_IMG.ToString(); ;
 ...
 if (MSG.Msg.MsgBoxResult(CLS.Q_MSG_SAVE))
                {
                    ...
                    if (CLS.SUBMIT_FLAG == 1)
                    {
                        ...
                        UPD_STATE_IMGE.P_IN_ACTIVE_IMG_ADD_STATE(Convert.ToInt32(CLS.USER_CODE), Convert.ToInt32(LBL_BOOK_CODE.Text), 0, ref CLS.SUBMIT_FLAG);
