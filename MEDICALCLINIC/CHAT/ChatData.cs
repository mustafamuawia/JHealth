...
namespace MEDICALCLINIC.CHAT
{
    ...
    {
        private static string m_ConnectionString = Properties.Settings.Default["MEDICAL_CLINIC_DBConnectionString"].ToString();
        private SqlConnection m_sqlConn = null;
        //public delegate void OnCallEventHandler(object sender);
        ...
        public DataTable GetLMessages()
        {
            ...
            try
            {
                ...
                // SELECT <field> FROM Table
                // Query also can not use *, fields must be designated
                SqlCommand cmd = new SqlCommand("usp_GetLMessages", m_sqlConn);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable GetWAITNAMES()
        {
            ...
            try
            {
                ...
                // SELECT <field> FROM Table
                // Query also can not use *, fields must be designated
                SqlCommand cmd = new SqlCommand("[GET_BOOKDOCTOR_INFO_SHOW_1", m_sqlConn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@ID_DOCTOR", SqlDbType.Int).Value = 2112001;
